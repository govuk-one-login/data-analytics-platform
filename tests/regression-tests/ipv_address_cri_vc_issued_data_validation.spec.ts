...
describe('IPV_ADDRESS_CRI_VC_ISSUED Data validation Test', () => {
  test.concurrent.each`
    eventName                    | event_id               | client_id              | journey_id
      ${'IPV_ADDRESS_CRI_VC_ISSUED'}     | ${faker.string.uuid()} | ${faker.string.uuid()} | ${faker.string.uuid()}
    `(
  ...
  function validateDataAtStageAndConformed(rawData, stageData, redShiftQueryResults): void {
    const addressesentered = rawData.addressesentered;
    if (addressesentered !== 'null' && addressesentered !== null && addressesentered !== undefined) {
      const stageBiometricverificationprocesslevel = stageData.extensions_addressesentered;
      expect(addressesentered).toEqual(stageBiometricverificationprocesslevel);
      expect(stageBiometricverificationprocesslevel).toEqual(redShiftQueryResults.Records[0][3].stringValue);
    }
    const isss = rawData.iss;
    if (isss !== 'null' && isss !== null && isss !== undefined) {
      const stageIss = stageData.extensions_iss;
      expect(isss).toEqual(stageIss);
      expect(stageIss).toEqual(redShiftQueryResults.Records[0][2].stringValue);
    }
