...
describe('Raw to Stage Integration Tests', () => {
  test.each(getTestEventPairs())(
    'Event $auditEvent.event_name ($auditEvent.event_id) should be present in Athena stage layer table',
  ...
  test.each(getTestEventPairs())(
    'Event $auditEvent.event_name ($auditEvent.event_id) should be present in Athena stage layer key values table',
