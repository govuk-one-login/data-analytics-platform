...
### Sequential group (run in order after parallel group completes)

| File | Stored Procedure |
|------|-----------------|
| `redshift_fact_user_journey_event_refactored_upsert.asl.json` | `conformed_refactored.fact_user_journey_event_refactored_upsert()` |

The following two procedures run inline within the parent orchestrator (not as child state machines):

1. `conformed_refactored.event_extensions_refactored_upsert()`
2. `conformed_refactored.update_event_batch_table()`
...
└── 3. txma_redshift_consolidated_schema_processing (sync)
    ...
    │
    ├── 3b. redshift_fact_user_journey_event_refactored_upsert
