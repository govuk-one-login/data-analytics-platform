...
BEGIN
        ...
        values(7,'event_extensions_refactored_upsert','started',GETDATE());
...
INSERT INTO  conformed_refactored.event_extensions_refactored ( user_journey_event_key,Parent_attribute_name,event_attribute_name,event_attribute_value,event_id,
 ...
 SELECT distinct
        ...
        FROM  conformed_refactored.fact_user_journey_event_refactored fct
         join  (SELECT event_id,parent_column_name,key,value,processed_dt,processed_time from
              (SELECT *,row_number()over(partition by event_id,parent_column_name,key,value order by processed_dt desc,processed_time) row_nm
        ...
        AND TO_TIMESTAMP(
                        LEFT(stage_key_val.processed_dt, 4) || '-' || 
                        SUBSTRING(stage_key_val.processed_dt, 5, 2) || '-' || 
                        RIGHT(stage_key_val.processed_dt, 2) || ' ' || 
                        LPAD(LEFT(LPAD(stage_key_val.processed_time, 6, '0'), 2), 2, '0') || ':' || 
                        LPAD(SUBSTRING(LPAD(stage_key_val.processed_time, 6, '0'), 3, 2), 2, '0') || ':' || 
                        LPAD(RIGHT(LPAD(stage_key_val.processed_time, 6, '0'), 2), 2, '0'),
...
UPDATE  conformed_refactored.event_extensions_refactored
    ...
    FROM ( SELECT distinct 
       fct.user_journey_event_key,parent_column_name Parent_attribute_name,key event_attribute_name ,value event_attribute_value,stage_key_val.event_id
        ...
        on ext.user_journey_event_key= fct.user_journey_event_key 
        and ext.event_attribute_name=stage_key_val.key  
        and ext.event_attribute_value<>stage_key_val.value)AS st
