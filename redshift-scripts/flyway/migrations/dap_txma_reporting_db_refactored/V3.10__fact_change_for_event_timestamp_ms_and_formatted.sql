...
SELECT NVL(DE.event_key,-1) AS event_key
    ...
    FROM  (SELECT timestamp_formatted,component_id,stg.partition_event_name,client_id,stg.user_user_id
           ,stg.user_govuk_signin_journey_id,stg.event_id,processed_dt,event_timestamp_ms,event_timestamp_ms_formatted
      FROM  (SELECT * FROM
             (SELECT timestamp_formatted,component_id,partition_event_name ,client_id,user_user_id
              ,user_govuk_signin_journey_id,event_id,processed_dt,event_timestamp_ms,event_timestamp_ms_formatted,
