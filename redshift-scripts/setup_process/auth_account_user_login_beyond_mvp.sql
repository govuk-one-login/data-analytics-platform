CREATE OR replace PROCEDURE conformed.sp_auth_account_user_login_beyond_mvp ()
AS $$
BEGIN  
...
FROM (
  SELECT *
  FROM conformed.v_stg_auth_account_user_login_beyond_mvp_one_time
  WHERE EVENT_NAME IN (
    SELECT EVENT_NAME
...
SELECT DISTINCT CASE 
    ...
    CURRENT_DATE,
    9999
FROM conformed.v_stg_auth_account_user_login_beyond_mvp_one_time AS st
WHERE (CASE 
        WHEN st.EVENT_NAME LIKE '%IPV%' THEN 'Web'
        WHEN st.EVENT_NAME LIKE '%DCMAW%' THEN 'App'
        ELSE 'General'
    ...
    FROM ( select DISTINCT
            ...
            CURRENT_DATE,
            9999
            FROM conformed.v_stg_auth_account_user_login_beyond_mvp_one_time mn
 ...
 INSERT INTO conformed.FACT_USER_JOURNEY_EVENT (EVENT_KEY,DATE_KEY,verification_route_key,journey_channel_key,relying_party_key,USER_USER_ID,
    ...
    SELECT NVL(DE.event_key,-1) AS event_key
         ...
         -- ,FAILED_CHECK_DETAILS_CHECK_METHOD
           ...
           --,CAST(CHECK_DETAILS_CHECK_METHOD AS INTEGER)
  ...
  INSERT into audit.err_duplicate_event_id_auth_account_user_login_3 (total_duplicate_event_count_minus_one
  ...
  FROM
      (
          ...
          FROM
              (
                  SELECT
                      'auth_account_user_login' AS Product_family,
          ...
          AND (auth.product_family,event_name, event_id) NOT IN (SELECT product_family ,event_name, event_id 
                                                                      FROM audit.err_duplicate_event_id_auth_account_user_login_3)
          GROUP BY
              auth.Product_family,
              event_name,
              event_id,
	...
	raise info 'processing of product family: auth_account_user_login_beyond_mvp ran successfully';

	EXCEPTION WHEN OTHERS THEN 
        RAISE EXCEPTION '[error while processing product family: auth_account_user_login_beyond_mvp] exception: %',sqlerrm;
