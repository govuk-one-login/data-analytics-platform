CREATE OR replace PROCEDURE conformed.sp_ipv_cri_address_beyond_mvp ()
AS $$
BEGIN  
...
FROM (
  SELECT *
  FROM conformed.v_stg_ipv_cri_address_beyond_mvp_one_time
  WHERE EVENT_NAME IN (
    SELECT EVENT_NAME
...
SELECT DISTINCT CASE 
    ...
    CURRENT_DATE,
    9999
FROM conformed.v_stg_ipv_cri_address_beyond_mvp_one_time AS st
WHERE (CASE 
        WHEN st.EVENT_NAME LIKE '%IPV%' THEN 'Web'
        WHEN st.EVENT_NAME LIKE '%DCMAW%' THEN 'App'
        ELSE 'General'
...
FROM (
        ...
        CURRENT_DATE,
        9999
        FROM conformed.v_stg_ipv_cri_address_beyond_mvp_one_time mn
...
UPDATE "dap_txma_reporting_db"."conformed"."fact_user_journey_event"
    ...
    INSERT into audit.err_duplicate_event_id_ipv_cri_address_7 (total_duplicate_event_count_minus_one
    ...
    FROM
        (
            ...
            FROM
                (
                    SELECT
                        'ipv_cri_address' AS Product_family,
            ...
            AND (auth.product_family,event_name, event_id) NOT IN (SELECT product_family ,event_name, event_id 
                                                                        FROM audit.err_duplicate_event_id_ipv_cri_address_7)
            GROUP BY
                auth.Product_family,
                event_name,
                event_id,
	...
	raise info 'processing of product family: ipv_cri_address_beyond_mvp ran successfully';

	EXCEPTION WHEN OTHERS THEN 
        RAISE EXCEPTION '[error while processing product family: ipv_cri_address_beyond_mvp] exception: %',sqlerrm;
