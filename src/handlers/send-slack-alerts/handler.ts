...
export const handler = async (event: any): Promise<void> => { // Accept any event type
  try {
    // Check if the event is an array of S3 events
    if (Array.isArray(event.Records) && event.Records.some((record: { eventSource: string; }) => record.eventSource === "aws:s3")) {
      for (const record of event.Records) {
        // Process only S3 replication events
        if (record.awsRegion === "eu-west-2" && // Replace with your replication region
            record.eventName && 
