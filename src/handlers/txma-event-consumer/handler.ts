...
const processRecord = async (record: SQSRecord): Promise<boolean> => {
  try {
    ...
    if (!isValidAuditEvent(auditEvent)) {
      // @ts-ignore
      logger.error('Invalid audit event', { eventId: auditEvent.event_id });
      return false;
    }
    ...
    logger.info('Successfully processed audit event', { eventId: auditEvent.event_id });
    return true;
  } catch (e) {
    logger.error('Error processing record', { body: record.body, error: e });
    return false;
  }
};
