...
invalid_field_structure_msg = "Invalid field list structure provided, require list object"


def try_to_extract_numbers_from_str(value):
...
def empty_string_to_null(df, fields):
    ...
    try:
        if not isinstance(fields, list):
            raise ValueError(invalid_field_structure_msg)
...
class DataPreprocessing:
    ...
    def add_new_column(self, df, fields):
        ...
        try:
            ...
            for column_name in fields.items():
            ...
            for key, value in fields.items():
                for item in value:
                    ...
                    df[col_name] = df.apply(
                        lambda x, i=item, k=key: (None if x[k] is None or x[k].get(i) is None or (not x[k].get(i).strip()) else x[k].get(i)),
                        axis=1,
                    )
            ...
            if not isinstance(obj, (dict, list)):
                ...
                for key, value in obj.items():
                    if value is not None:
                        self.extract_values_by_type_of_ds(items, key, parent_key, sep, value)
            return items
    ...
    def extract_values_by_type_of_ds(self, items, key, parent_key, sep, value):
        ...
        if isinstance(value, dict):
            ...
            value = try_to_extract_numbers_from_str(value)
