...
class ErrorHandler:
    ...
    def write_failed_records_to_s3(self):
        ...
        try:
            ...
            # Add metadata
            combined_errors["error_timestamp"] = datetime.now().isoformat()
            combined_errors["error_batch_id"] = datetime.now().strftime("%Y%m%d_%H%M%S")

            # Convert to JSON
            error_json = combined_errors.to_json(orient="records", date_format="iso")

            # Generate S3 key
            error_key = f"transformation-errors/{datetime.now().strftime('%Y/%m/%d')}/errors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
