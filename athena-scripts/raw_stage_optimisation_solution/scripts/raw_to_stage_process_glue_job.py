...
def process_results(dfs, args, preprocessing, json_data, glue_app, s3_app):
    ...
    for df_raw in dfs:
        ...
        df_raw = remove_rows_missing_mandatory_values(
            preprocessing, json_data, df_raw)
        if df_raw is None:
            raise ValueError(
        ...
        if df_raw is None:
            raise ValueError(
                f"Function: add_new_column_from_struct returned None.")
        ...
        cummulative_stage_table_rows_inserted = cummulative_stage_table_rows_inserted + \
            int(len(df_raw))
        ...
        df_keys = generate_key_value_records(
            preprocessing, json_data, df_raw, stage_key_value_schema_columns, df_raw_col_names_original)

        if df_keys is None:
            raise ValueError(
                f"Function: generate_key_value_records returned None.")
        ...
        cummulative_stage_key_rows_inserted = cummulative_stage_key_rows_inserted + \
            int(len(df_keys))
        ...
        try:
            ...
            if not stage_table_update:
                sys.exit(
                    "Update to stage table did not return boolean(True) response")
            ...
            if http_response is None:
                sys.exit(
                    "Insert of stage table metadata returned invalid response")
...
def main():
    ...
    try:
        ...
        if raw_sql_select is None:
            raise ValueError(
                "Function 'generate_raw_select_filter' returned None, which is not allowed.")
        print(f'raw layer sql filter: {raw_sql_select}')

        # Query raw data
        dfs = glue_app.query_glue_table(
            args['raw_database'], raw_sql_select, athena_query_chunksize)
        if dfs is None:
            raise ValueError(f"Function: query_glue_table returned None.  Using query {str(raw_sql_select)}")

        # Process all new raw data
        process_results(dfs, args, preprocessing, json_data, glue_app, s3_app)

        '''
        # At this point, the usual ETL job has processed successfully...
        # Due to a couple of scenarios where events arrive late in to dap or crawler doesnt pick them up at all, we 
        # need to look back at the previous job and check if there are any events that are missing during that period that
        # should be in the stage layer. These events will be loaded during this run ensuring eventual consistency
        #     
        # Currently, this part of the process will not fail if any exception is thrown
        # 
        '''
        ...
        if filter_processed_dt == preprocessing.processed_dt:
            current_process_time = preprocessing.processed_time

        all_previous_processed_times = get_all_processed_times_per_day(
            glue_app, args['stage_database'], args['stage_target_table'], filter_processed_dt, current_process_time)
        ...
        all_previous_processed_dts = get_all_processed_dts(glue_app,   
                                                          args['stage_database'],
                                                          args['stage_target_table'], 
                                                          filter_processed_dt, 
                                                          preprocessing.processed_dt)
        ...
        min_timestamp_filter_for_missing_events = get_min_timestamp_from_previous_run(all_previous_processed_times,
                                                                                      glue_app,
                                                                                      args['stage_database'],
                                                                                      args['stage_target_table'],
                                                                                      filter_processed_dt,
        ...
        raw_sql_select = generate_missing_event_ids_select_filter(args['raw_database'],
                                                                  args['stage_database'],
                                                                  filter_processed_dt,
        ...
        if raw_sql_select is None:
            print(f'Could not generate select filter for missing events, ending process')
            return

        print(f'raw layer missing event ids filtr: {raw_sql_select}')
        ...
        # Process any missing events - dont fail process if there is an error
        try:
            process_results(dfs, args, preprocessing, json_data, glue_app, s3_app)
        except Exception as e:
            print(f'error adding missing events to stage layer {str(e)}, ending process')
            return
