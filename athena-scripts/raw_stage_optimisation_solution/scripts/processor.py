...
def process_job(json_data, args, glue_app, s3_app, data_preprocessing):
    ...
    stage_target_table = args['stage_target_key_values']
    ...
    if job_type is 'TESTING':
        ...
        # query raw layer
        dfs = get_raw_data(glue_app, raw_database, testing_raw_sql, athena_query_chunksize)
        # process results
        ...
        max_processed_dt = get_max_processed_dt(glue_app, raw_database, raw_table, stage_database, stage_target_table)
        ...
        if max_processed_dt == glue_app.processed_dt:
            current_process_time = data_preprocessing.processed_time

        all_previous_processed_times = get_all_processed_times_per_day(
            glue_app, args['stage_database'], args['stage_target_table'], max_processed_dt, current_process_time)
...
def get_raw_sql_scheduled(json_data, database, table, filter_processed_dt, filter_timestamp):
...
def get_raw_sql_backfill(raw_database, raw_table, stage_layer_database, stage_layer_target_table, filter_min_timestamp, filter_max_timestamp, filter_processed_dt, filter_processed_time, penultimate_processed_dt):
