CREATE OR REPLACE VIEW "raw_database"."txma_and_splunk_dedup_view_batch_7" AS 

WITH splunk_migration AS (
	SELECT *
	FROM "raw_database"."splunk_migration"
	WHERE partition_0 in (
'AUTH_REAUTHENTICATION_INVALID', 'AUTH_REAUTHENTICATION_SUCCESSFUL', 'AUTH_UPDATE_PASSWORD', 'AUTH_UPDATE_PROFILE_REQUEST_RECEIVED', 'CIC_CRI_END', 'DCMAW_ABORT_WEB', 'DCMAW_APP_HANDOFF_START', 'DCMAW_BRP_SELECTED', 'F2F_CRI_AUTH_CODE_ISSUED', 'F2F_YOTI_START', 'HOME_TRIAGE_PAGE_EMAIL', 'IPV_CONTRA_INDICATOR_STORAGE_MITIGATIONS_PUT_START', 'IPV_CORE_CRI_RESOURCE_RETRIEVED', 'IPV_CRI_AUTH_RESPONSE_RECEIVED', 'IPV_DL_CRI_RESPONSE_RECEIVED', 'IPV_DL_CRI_VC_ISSUED', 'IPV_F2F_CRI_VC_CONSUMED', 'IPV_FRAUD_CRI_END', 'IPV_FRAUD_CRI_RESPONSE_RECEIVED', 'IPV_KBV_CRI_ABANDONED', 'IPV_SPOT_REQUEST_RECEIVED', 'IPV_VC_RECEIVED', 'TICF_ACCOUNT_INTERVENTION'   		)
