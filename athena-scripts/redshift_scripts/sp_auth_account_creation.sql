...
*/
    ...
    -- Not needed as once replying party is inserted no need to update that.
  ...
  INSERT INTO conformed.FACT_USER_JOURNEY_EVENT (EVENT_KEY,DATE_KEY,verification_route_key,journey_channel_key,relying_party_key,USER_USER_ID,
    ...
    INSERT into audit.err_duplicate_event_id_auth_account_creation_1 (total_duplicate_event_count_minus_one
    ...
    FROM
        (
            ...
            AND (auth.product_family,event_name, event_id) NOT IN (SELECT product_family ,event_name, event_id 
                                                                        FROM audit.err_duplicate_event_id_auth_account_creation_1)
            GROUP BY
                auth.Product_family,
                event_name,
                event_id,
