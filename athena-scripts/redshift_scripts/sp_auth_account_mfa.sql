...
*/
          ...
          ,EVENT_COUNT
           ...
           ,trim(CASE when CHECK_DETAILS_BIOMETRIC_VERIFICATION_PROCESS_LEVEL='null'
                                        ...
                                        ELSE
                                            CHECK_DETAILS_BIOMETRIC_VERIFICATION_PROCESS_LEVEL
                                        END,'"')
           --,CAST(ADDRESSES_ENTERED AS INTEGER)
           ...
           ,trim(CASE when FAILED_CHECK_DETAILS_CHECK_METHOD='null'
                                        ...
                                        ELSE
                                            FAILED_CHECK_DETAILS_CHECK_METHOD
                                        END,'"')
           --,CAST(CHECK_DETAILS_KBV_RESPONSE_MODEL AS INTEGER)
           ...
           ,trim(CASE when CHECK_DETAILS_KBV_RESPONSE_MODEL='null'
                                        ...
                                        END,'"')
           --,CAST(CHECK_DETAILS_KBV_QUALITY AS INTEGER)
  ...
  INSERT into audit.err_duplicate_event_id_auth_account_mfa_5 (total_duplicate_event_count_minus_one
  ...
  FROM
      (
          ...
          FROM
              (
                  SELECT
                      'auth_account_mfa' AS Product_family,
          ...
          AND (auth.product_family,event_name, event_id) NOT IN (SELECT product_family ,event_name, event_id 
                                                                      FROM audit.err_duplicate_event_id_auth_account_mfa_5)
          GROUP BY
              auth.Product_family,
              event_name,
              event_id,
