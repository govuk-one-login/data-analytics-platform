CREATE OR replace PROCEDURE conformed.sp_ipv_cri_cic ()
AS $$
BEGIN  
...
FROM (
  SELECT *
  FROM conformed.v_stg_ipv_cri_cic
  WHERE EVENT_NAME IN (
    SELECT EVENT_NAME
...
SELECT DISTINCT CASE 
    ...
    CURRENT_DATE,
    9999
FROM conformed.v_stg_ipv_cri_cic AS st
WHERE (CASE 
        WHEN st.EVENT_NAME LIKE '%IPV%' THEN 'Web'
        WHEN st.EVENT_NAME LIKE '%DCMAW%' THEN 'App'
        ELSE 'General'
...
SET 
  ...
  ,ACTIVITY_HISTORY_SCORE=
                        CAST(case when st.ACTIVITY_HISTORY_SCORE ~ '^[0-9]+$' then st.ACTIVITY_HISTORY_SCORE
                              else null
                              end AS INTEGER)
  ,IDENTITY_FRAUD_SCORE=
                        CAST(case when st.IDENTITY_FRAUD_SCORE ~ '^[0-9]+$' then st.IDENTITY_FRAUD_SCORE
                              else null
                              end AS INTEGER)
  ,DECISION_SCORE=
                CAST(case when st.DECISION_SCORE ~ '^[0-9]+$' then st.DECISION_SCORE
                              else null
                              end AS INTEGER)
  ...
  ,VERIFICATION_SCORE=
                    CAST(case when st.VERIFICATION_SCORE ~ '^[0-9]+$' then st.VERIFICATION_SCORE
                              else null
                              end AS INTEGER)
  ...
  ,Iss=st.Iss
  ,VALIDITY_SCORE=
                  CAST(case when st.VALIDITY_SCORE ~ '^[0-9]+$' then st.VALIDITY_SCORE
                              else null
                              end AS INTEGER)
  ,"TYPE"=
...
SELECT NVL(DE.event_key,-1) AS event_key
      ...
      ,EVENT_COUNT
       ...
       --,ACTIVITY_HISTORY_SCORE
       ,CAST(case when ACTIVITY_HISTORY_SCORE ~ '^[0-9]+$' then ACTIVITY_HISTORY_SCORE
            else null
            end AS INTEGER)
       --,IDENTITY_FRAUD_SCORE
       ,CAST(case when IDENTITY_FRAUD_SCORE ~ '^[0-9]+$' then IDENTITY_FRAUD_SCORE
            else null
            end AS INTEGER)
       --,DECISION_SCORE
       ,CAST(case when DECISION_SCORE ~ '^[0-9]+$' then DECISION_SCORE
            else null
            end AS INTEGER)
       --,FAILED_CHECK_DETAILS_KBV_RESPONSE_MODE
       ...
       --,VERIFICATION_SCORE
       ,CAST(case when VERIFICATION_SCORE ~ '^[0-9]+$' then VERIFICATION_SCORE
            else null
            end AS INTEGER)
       --,CHECK_DETAILS_CHECK_METHOD
       ...
       ,Iss
       --,VALIDITY_SCORE
       ,CAST(case when VALIDITY_SCORE ~ '^[0-9]+$' then VALIDITY_SCORE
            else null
            end AS INTEGER)
       --,"TYPE"
...
LEFT JOIN conformed.dim_verification_route dvr 
    ...
    INSERT into audit.err_duplicate_event_id_ipv_cri_cic_13 (total_duplicate_event_count_minus_one
    ...
    FROM
        (
            ...
            AND (auth.product_family,event_name, event_id) NOT IN (SELECT product_family ,event_name, event_id 
                                                                        FROM audit.err_duplicate_event_id_ipv_cri_cic_13)
            GROUP BY
                auth.Product_family,
                event_name,
                event_id,
	...
	raise info 'processing of product family: sp_ipv_cri_cic ran successfully';

	EXCEPTION WHEN OTHERS THEN 
        RAISE EXCEPTION '[error while processing product family: sp_ipv_cri_cic] exception: %',sqlerrm;
