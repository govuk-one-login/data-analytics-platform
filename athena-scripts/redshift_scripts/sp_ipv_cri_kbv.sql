CREATE OR replace PROCEDURE conformed.sp_ipv_cri_kbv ()
AS $$
BEGIN
/*
Name       Date         Notes
...
*/
    ...
    FROM (
      SELECT *
      FROM conformed.v_stg_ipv_cri_kbv
      WHERE EVENT_NAME IN (
        SELECT EVENT_NAME
    ...
    SELECT DISTINCT CASE 
        ...
        CURRENT_DATE,
        9999
    FROM conformed.v_stg_ipv_cri_kbv AS st
    WHERE (CASE 
            WHEN st.EVENT_NAME LIKE '%IPV%' THEN 'Web'
            WHEN st.EVENT_NAME LIKE '%DCMAW%' THEN 'App'
            ELSE 'General'
   ...
   /* UPDATE conformed.DIM_RELYING_PARTY
    ...
    FROM (
            ...
            CURRENT_DATE,
            9999
            FROM conformed.v_stg_ipv_cri_kbv mn
    ...
    SET 
      ...
      ,FAILED_CHECK_DETAILS_CHECK_METHOD=--json_serialize(st.FAILED_CHECK_DETAILS_CHECK_METHOD)
                                        ...
                                        END ,'"')
      ,CHECK_DETAILS_KBV_RESPONSE_MODE=--trim(json_serialize(st.CHECK_DETAILS_KBV_RESPONSE_MODEL) ,'"') 
                                        trim(CASE when json_serialize(st.CHECK_DETAILS_KBV_RESPONSE_MODEL)='null'
                                        then NULL
                                        ELSE
                                            json_serialize(st.CHECK_DETAILS_KBV_RESPONSE_MODEL)
                                        END ,'"')
      ,CHECK_DETAILS_KBV_QUALITY=--trim(json_serialize(st.CHECK_DETAILS_KBV_QUALITY) ,'"') 
      ...
      ,CHECK_DETAILS_CHECK_METHOD=--json_serialize(st.CHECK_DETAILS_CHECK_METHOD)
                                trim(CASE when json_serialize(st.CHECK_DETAILS_CHECK_METHOD)='null'
                                        ...
                                        END ,'"')
      ,Iss=st.Iss
      ,EXPERIAN_IIQ_RESPONSE=--st.EXPERIANIIQRESPONSE
                            trim(CASE when st.EXPERIANIIQRESPONSE='null'
                                        then NULL
                                        ELSE
                                            st.EXPERIANIIQRESPONSE
                                        END ,'"')
  ...
  INSERT INTO conformed.FACT_USER_JOURNEY_EVENT (EVENT_KEY,DATE_KEY,verification_route_key,journey_channel_key,relying_party_key,USER_USER_ID,
    ...
    SELECT NVL(DE.event_key,-1) AS event_key
          ...
          ,EVENT_COUNT
           ...
           ,trim(CASE when json_serialize(FAILED_CHECK_DETAILS_CHECK_METHOD)='null'
                                        ...
                                        ELSE
                                            json_serialize(FAILED_CHECK_DETAILS_CHECK_METHOD)
                                        END ,'"')
           --,trim(json_serialize(CHECK_DETAILS_KBV_RESPONSE_MODEL) ,'"')
           ,trim(CASE when json_serialize(CHECK_DETAILS_KBV_RESPONSE_MODEL)='null'
                                        then NULL
                                        ELSE
                                            json_serialize(CHECK_DETAILS_KBV_RESPONSE_MODEL)
                                        END ,'"')
          --,trim(json_serialize(CHECK_DETAILS_KBV_QUALITY) ,'"')
          ,trim(CASE when json_serialize(CHECK_DETAILS_KBV_QUALITY)='null'
                                        ...
                                        END ,'"')
           --,VERIFICATION_SCORE
           --,CAST(json_serialize(VERIFICATION_SCORE) AS INTEGER)
           ,CAST(case when trim(json_serialize(VERIFICATION_SCORE),'"') ~ '^[0-9]+$' then trim(json_serialize(VERIFICATION_SCORE),'"') 
            else null
            end AS INTEGER)
           --,trim(json_serialize(CHECK_DETAILS_CHECK_METHOD),'"')
           ,trim(CASE when JSON_SERIALIZE(CHECK_DETAILS_CHECK_METHOD) ='null'
                 then NULL
                 ELSE
                    JSON_SERIALIZE(CHECK_DETAILS_CHECK_METHOD) 
                 END,'"')   CHECK_DETAILS_CHECK_METHOD 
           ,Iss
           --,EXPERIANIIQRESPONSE
           ,trim(CASE when EXPERIANIIQRESPONSE='null'
                                        then NULL
                                        ELSE
                                            EXPERIANIIQRESPONSE
                                        END ,'"')
    ...
    INSERT into audit.err_duplicate_event_id_ipv_cri_kbv_11 (total_duplicate_event_count_minus_one
    ...
    FROM
        (
            ...
            AND (auth.product_family,event_name, event_id) NOT IN (SELECT product_family ,event_name, event_id 
                                                                        FROM audit.err_duplicate_event_id_ipv_cri_kbv_11)
            GROUP BY
                auth.Product_family,
                event_name,
                event_id,
	...
	raise info 'processing of product family: ipv_cri_kbv ran successfully';

	EXCEPTION WHEN OTHERS THEN 
        RAISE EXCEPTION '[error while processing product family: ipv_cri_kbv] exception: %',sqlerrm;
